...
| Skill | Carries |
|---|---|
| `credit-360-cockpit` | the OPEN (hand over `assets/cockpit.json`'s `canonicalArtifactUrl`, no fetch and no publish) and the REBUILD (the fetch sequence, the `C360_DATA` contract, the assembler, the render) |
