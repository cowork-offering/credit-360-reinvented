...
import { fmtCovThreshold, fmtCovVal, fmtRatio } from "../data/finance";
...
/* =============================================================================
   ONE CONTEXT BUILDER FOR THE WHOLE RELATIONSHIP (backlog item 9, golden rule 1).

   THE DEFECT THIS ENDS. Three builders read one bundle and disagreed: the
   workroom envelope (`components/workroom/readBlocks.ts`), the cockpit chat
   (`channel/deskAsk.ts`) and the relationship room (`components/relationship/
   relBrain.ts`). On Hartwell the desk said $57M across 9 facilities and the room
   said $49M across 7, both honest and neither reconcilable from the other side;
   the desk carried maturities and stages the envelope dropped, and the envelope
   carried thresholds, collateral and parties the desk dropped. A banker who
   asked the same question on two surfaces got two books.

   THE FIX IS ONE PRODUCTION, MANY SELECTIONS. Every fact is built here, once,
   in the room's own printed form. Each surface then SELECTS from it against its
   own budget, in ONE drop order ({@link CONTEXT_DROP_ORDER}), and refuses what
   no read carries through ONE `notCarried` derivation. Two surfaces can still
   carry different AMOUNTS of the book; they can no longer carry different FACTS,
   and a total is never quoted without the scope it was summed over.

   IT FORMATS; IT NEVER DERIVES. Every figure is printed by the same helpers the
   glass prints with (`fmtMoney`, `fmtDate`, `fmtRatio`, `classifyCovenant`), so
   an answer and the card beside it can never disagree about a number.
   ============================================================================= */

/** The relationship-wide scope word, used wherever a row hangs off no facility. */
const RELATIONSHIP = "across the relationship";

/** THE ONE DROP ORDER. Read blocks in the order they are given up, first to
 *  last. The envelope drops from the front of this list; the cockpit chat emits
 *  its parts in the REVERSE of it, so the block the envelope surrenders first is
 *  the block the chat's own cut reaches first. Exposure is last on both: it is
 *  four figures and it grounds nearly every question a banker asks.
 *
 *  Every block the builder can emit must be on this list, or it is the one
 *  block a cap can never give up. */
...
/** What NO read on this cockpit carries, stated the same way every time and on
 *  every surface.
 *
...
/** What the cockpit holds of the CORRESPONDENCE, said only where the caller
...
/* --------------------------------------------------------------- the facilities
   ...
   WHAT A FACILITY IS, not just what it is called. The envelope carried a label
   and a commitment, so "what does this facility do" could not be answered with
   its maturity, its drawn balance, its stage or its own coverage: all four sat
   on the same read the cockpit chat was already printing them from. One block,
   one naming, both surfaces.                                                  */

function facilityBlock(src: ReadSource): BrainReadBlocks["facilities"] {
  const facilities = scoped(src);
  if (!facilities.length) return undefined;
  return facilities.map((f) => ({
    name: nameOf(f, src.accountName, facilities),
    commitment: typeof f.committed === "number" ? fmtMoney(f.committed) : undefined,
    drawn: typeof f.outstanding === "number" ? fmtMoney(f.outstanding) : undefined,
    available: typeof f.available === "number" ? fmtMoney(f.available) : undefined,
    rate: typeof f.interestRate === "number" ? `${f.interestRate}%` : undefined,
    maturity: f.maturityDate ? fmtDate(f.maturityDate) : undefined,
    stage: f.stage ?? undefined,
    // The org's OWN per-facility ratio, through the formatter the exposure tab
    // prints it with. Never a ratio this builder worked out for itself.
    coverage: typeof f.coverageRatio === "number" ? fmtRatio(f.coverageRatio) : undefined,
  }));
}
...
function groupBlock(src: ReadSource): BrainReadBlocks["group"] {
  ...
  for (const c of best.values()) {
    ...
    rows.push({
      ...
      // THE ORG'S OWN ACCOUNT ID (backlog item 14b, 2026-09-12). `connectedPartyBook`
      // is bound to a counterparty the anchored graph names and resolves it by id;
      // without the id on the row the model could only ever address it by name,
      // which is the one handle two Hartwell entities share a prefix of.
      counterpartyId: c.counterpartyId ?? undefined,
      relation,
...
/** The version chain on ONE package, named. The envelope takes the row for the
 *  package it is anchored on; the cockpit chat, which is anchored on nothing,
 *  takes every row. Same facts, one derivation. */
export interface InFlightRow extends NonNullable<BrainReadBlocks["inFlight"]> {
  packageId: string;
  packageName: string;
}

/** Every package on the relationship that is a version, or has one in flight.
 *  Empty where the roster names none on either side. */
export function inFlightRoster(src: ReadSource): InFlightRow[] {
  const rows: InFlightRow[] = [];
  for (const entry of packageRoster(src.bundle, src.history)) {
    if (!entry.inFlightVersion && !entry.hasInFlightModification) continue;
    rows.push({
      packageId: entry.id,
...
function inFlightBlock(src: ReadSource): BrainReadBlocks["inFlight"] {
  ...
  const row = inFlightRoster(src).find((r) => r.packageId === src.productPackageId);
  if (!row) return undefined;
  const { packageId: _id, packageName: _name, ...block } = row;
  return block;
}
...
function exposureBlock(src: ReadSource): BrainReadBlocks["exposure"] {
  ...
  return {
    ...
    // facility on the relationship and a workroom totals the anchored package:
    // on Hartwell that is $57M against $49M, both honest, and neither used to
    // say which. A total with no scope on it is how one deal reads as two.
...
/* ------------------------------------------------------------- the trail
   ...
   WHAT THIS COCKPIT HAS ALREADY FILED on this relationship. The host has passed
   it to the builder since the envelope shipped (`ReadSource.history`) and the
   builder discarded it, so "what did we do last time" was answered by a model
   that had never been shown the answer. The last few rows only: the trail is
   unbounded and the budget is not.                                           */

/** How many filed actions travel. Three is the last round of work on the deal. */
const HISTORY_ROWS = 3;

function historyBlock(src: ReadSource): BrainReadBlocks["history"] {
  const rows = (src.history ?? [])
    .filter((r) => (r.summary ?? "").trim().length > 0)
    .slice(0, HISTORY_ROWS)
    .map((r) => ({
      what: (r.summary ?? "").trim(),
      action: r.actionId ?? undefined,
      status: r.status ?? undefined,
    }));
  return rows.length ? rows : undefined;
}

/** THE HONESTY LIST AS ONE SENTENCE, for a surface that speaks prose rather
 *  than carrying a `notCarried` array. Same list, same words, said standing
 *  rather than only when a cut happens to name it. */
export function notCarriedSentence(notCarried: readonly string[]): string {
  if (!notCarried.length) return "";
  return `This view does not carry ${notCarried.join("; ")}. Refuse those by name; never read their absence as a fact about this borrower.`;
}

/**
 * THE WHOLE RELATIONSHIP, READ ONCE.
 *
 * Absent where the caller stands on no read: an absent block is never an empty
 * fact, and a builder with no bundle has nothing to be honest about.
 *
 * `hasMail` adds the one refusal that only applies where a message travels
 * beside the context, which is the caller's own knowledge and not a read.
 */
export function buildRelationshipFacts(src: ReadSource | undefined, hasMail = false): BrainReadBlocks | undefined {
